...
from api.sql import Member, TRANSACTION, Product, Record, Cart, Browse
...
def bookstore():
    ...
    if 'keyword' in request.args and 'page' in request.args:
        ...
        # Record browse event (mid, pid, timestamp). Don't fail the view if logging fails.
        try:
            time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            Browse.add_browse(current_user.id, pid, time)
        except Exception as e:
            # Log to console for debugging; continue to render product page
            print(f"Failed to record browse: {e}")
...
def cart():
    ...
    if request.method == 'POST':
        if "pid" in request.form:
            ...
            # 檢查購物車裡面有沒有商品
            product = Record.check_product(pid, oid)
            # 取得商品價錢
            price = Product.get_product(pid)[2]

            # 如果購物車裡面沒有的話，把它加一個進去
            if product is None:
                Record.add_product({'pid': pid, 'oid': oid, 'saleprice': price, 'total': price})
            else:
                # 如果購物車裡面有的話，就多加一個進去
                amount = Record.get_amount(oid, pid)
                total = (amount + 1) * int(price)
                Record.update_product({'amount': amount + 1, 'oid': oid, 'pid': pid, 'total': total})

        elif "delete" in request.form:
            pid = request.form.get('delete')
            oid = Cart.get_cart(current_user.id)[2]

            Member.delete_product(oid, pid)
            ...
            oid = Cart.get_cart(current_user.id)[2]
            total = Record.get_total_money(oid)
            Cart.clear_cart(current_user.id)

            time = str(datetime.now().strftime('%Y/%m/%d %H:%M:%S'))
            format = 'yyyy/mm/dd hh24:mi:ss'
            TRANSACTION.add_order({'mid': current_user.id, 'ordertime': time, 'total': total, 'format': format, 'oid': oid})
...
def order():
    ...
    product_row = Record.get_record(oid)
    product_data = []
    ...
    total = float(Record.get_total(oid))  # 將 Decimal 轉換為 float
...
def change_order():
    data = Cart.get_cart(current_user.id)
    oid = data[2] # 使用者有購物車了，購物車的交易編號是什麼
